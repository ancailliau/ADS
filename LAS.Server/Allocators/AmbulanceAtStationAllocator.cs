...
namespace LAS.Server.Allocators
{
	...
	/// </summary>
	public class AmbulanceAtStationAllocator : IAmbulanceAllocator
	{
		...
		void Start()
		{
			foreach (var i in incidentsToProcess.GetConsumingEnumerable()) {
				...
				try {
					...
					GetClosestAmbulance(incidentPoint, ref closestAmbulance, ref timing, 
					                    ambulances.Where(x => x.Status != AmbulanceStatus.AvailableRadio));
